// markdown-it plugin for GitHub-style admonitions
// Transforms blockquotes starting with [!NOTE], [!TIP], [!IMPORTANT], [!WARNING], [!CAUTION]
...
module.exports = (md) => {
  ...
  md.core.ruler.after('block', 'admonition', (state) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      // Find the closing tag for this blockquote
      ...
      // Look for the first paragraph inside the blockquote
      for (let j = i + 1; j < closeIdx; j++) {
        if (tokens[j].type === 'paragraph_open' && tokens[j].level === level + 1) {
          ...
          if (inlineToken && inlineToken.type === 'inline' && inlineToken.content) {
            ...
            if (match) {
              ...
              const admonitionInfo = ADMONITION_TYPES[type];

              // Mark the blockquote as an admonition
              tokens[i].meta = tokens[i].meta || {};
              tokens[i].meta.admonition = {
                type: type.toLowerCase(),
                icon: admonitionInfo.icon,
                title: admonitionInfo.title
              };

              // Remove the [!TYPE] from the content
              inlineToken.content = inlineToken.content.replace(ALERT_RE, '').trim();

              // Also update children if they exist
              if (inlineToken.children && inlineToken.children.length > 0) {
                const firstChild = inlineToken.children[0];
                if (firstChild.type === 'text') {
                  firstChild.content = firstChild.content.replace(ALERT_RE, '').trim();
                }
              }
            }
          }
  ...
  md.renderer.rules.blockquote_open = (tokens, idx, options, env, self) => {
    ...
    if (token.meta && token.meta.admonition) {
      const { type, icon, title } = token.meta.admonition;
      return `<div class="admonition admonition-${type}">\n<p class="admonition-title">${icon}<span>${title}</span></p>\n`;
    }
    return defaultBlockquoteOpen(tokens, idx, options, env, self);
  };

  md.renderer.rules.blockquote_close = (tokens, idx, options, env, self) => {
    // Find the matching open token
    let level = tokens[idx].level;
    for (let i = idx - 1; i >= 0; i--) {
      if (tokens[i].type === 'blockquote_open' && tokens[i].level === level) {
        if (tokens[i].meta && tokens[i].meta.admonition) {
          return '</div>\n';
        }
        break;
      }
